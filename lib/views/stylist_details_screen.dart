...
class _StylistDetailScreenState extends State<StylistDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: CustomScrollView(
        slivers: [
          // Анимированный AppBar с фото стилиста в качестве фона
          SliverAppBar(
            expandedHeight: 250,
            pinned: true,
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          ' (${widget.stylist.comments.length} отзывов)',
                          style: TextStyle(
                    ...
                    _buildInfoRow(Icons.design_services, 'Специализация', widget.stylist.specialization),
                    const SizedBox(height: 8),
                    _buildInfoRow(Icons.location_on, 'Адрес', widget.stylist.address),
                    const SizedBox(height: 8),
                    _buildInfoRow(Icons.store, 'Салон', widget.stylist.salon),

                    // Опыт и информация
                    const SizedBox(height: 8),
                    _buildInfoRow(Icons.work, 'Опыт', '${widget.stylist.experience} лет'),
                    ...
                    if (widget.stylist.certificates.isNotEmpty) ...[
                      ...
                      const SizedBox(height: 8),
                      ...widget.stylist.certificates.map((cert) => Padding(
                    ...
                    if (widget.stylist.services.isNotEmpty) ...[
                      ...
                      ...widget.stylist.services.map((service) => _buildServiceItem(service)).toList(),
                    ],
                  ],
                ),
              ),
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
                  (context, index) {
                final comment = widget.stylist.comments[index];
                return ImprovedCommentCard(comment: comment);
              },
              childCount: widget.stylist.comments.length,
            ),
          ),
  ...
  // Диалог для записи к стилисту
  void _showBookingDialog(BuildContext context) {
    String selectedDate = widget.stylist.availableTimeSlots.keys.first;
    List<TimeSlot> availableSlots = widget.stylist.availableTimeSlots[selectedDate] ?? [];
    TimeSlot? selectedTimeSlot = availableSlots.isNotEmpty ? availableSlots.first : null;
    Service? selectedService = widget.stylist.services.isNotEmpty ? widget.stylist.services.first : null;
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButton<String>(
                  isExpanded: true,
                  value: selectedDate,
                  items: widget.stylist.availableTimeSlots.keys.map((date) {
                  ...
                  onChanged: (newValue) {
                    setState(() {
                      ...
                      availableSlots = widget.stylist.availableTimeSlots[selectedDate] ?? [];
                ...
                if (availableSlots.isEmpty)
                  ...
                  Wrap(
                    ...
                    children: availableSlots.map((slot) {
                      final startTime = '${slot.startTime.hour}:${slot.startTime.minute.toString().padLeft(2, '0')}';
                      return ChoiceChip(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: selectedTimeSlot != null && selectedService != null
                  ? () {
                // Здесь будет логика подтверждения записи
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('Вы записаны к стилисту ${widget.stylist.fullName} на ${selectedDate} в ${selectedTimeSlot!.startTime.hour}:${selectedTimeSlot!.startTime.minute.toString().padLeft(2, '0')}'),
  ...
  void _submitComment() {
    if (_commentController.text.trim().isNotEmpty) {
      // В реальном приложении здесь будет логика отправки комментария
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
